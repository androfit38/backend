...
class FitnessAssistant(Agent):
    ...
    """
    def __init__(self) -> None:  # Fixed: Added proper __init__ method
        super().__init__(
            instructions=(
...
async def entrypoint(ctx: agents.JobContext):
    await ctx.connect()  # Added connection to context
    ...
    session = AgentSession(
        ...
        llm=openai.LLM(
            model="gpt-4o-mini"  # Fixed: Changed from invalid "gpt-4.1-mini"
        ),
        tts=openai.TTS(
            model="tts-1",
            voice="alloy",
...
if __name__ == "__main__":  # Fixed: Added proper __name__ check
    try:
